...
/* A string problem. The intervals of occurunces can be calculated by iterating over the string and keeping count.*/
class Solution {
public:
    vector<vector<int>> largeGroupPositions(string s) {
        ...
        for (int i = 0; i < s.size(); i++) {
            int start = i;
            //Continuing to count while letters are same.
            while (s[i] == s[i+1]) {
                i++;
            }
            //If the letter makes a large group (Number of occurunces 3 or more) we store the interval in the vector ans.
            if ((i-start) >= 2) {
