...
/* 
    ...
    We can start to string using one of five different letter {a, e, i, o, u}.
    ...
    'a' should followed only by 'e'. Now we just need to find all permutations of a 4-length string starting with 'e'.
    'e' should followed by 'a' or 'i'. Now we need to find all permutations of a 3-length string starting with 'a' or 'i'.
    And so on.

    This problem can be solved using Dynamic Programming. I solved it using Bottom-Up approach.
    We store number of permutations for each letter starting with a 1-length string.
    Then for 2-length strings, we add the permutation count of 1-length to the possible letters. 
    By doing this iteratively up to n, we can reach to the n-length string for each letter.
    The result is equal to the sum of the permutation counts of the n-length strings for each letter.
...
public:
    ...
    int countVowelPermutation(int n) {
        ...
        // Store the permutations counts of each letter for 1...n length strings.
        // permutation[0][3] = 7. The permutation count of a 3-length string starting with 'a'. 
        vector<vector<int>> permutation(5, vector<int> (n+1));

        // Fill for 1-length string
        for (int i = 0; i < 5; i++) {
            permutation[i][1] = 1;
        }

        // Indexes of possible subsequent letters for each letter.
