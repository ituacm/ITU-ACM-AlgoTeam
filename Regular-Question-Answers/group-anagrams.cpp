   ...
   public:
    ...
    vector<vector<string>> groupAnagrams(vector<string>& strs) {
        ...
        for (int i = 0; i < strs.size(); i++) {
            ...
            // As I mentioned in this step every string  will be pushed its group vector
            mp[temp].push_back(strs[i]);
        }

        // Now I am traversing over the map and pushing that vector of strings to the two dimensional
        // vector that I have created before
        for (auto it : mp) {
