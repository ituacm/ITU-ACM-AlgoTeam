...
/* if a number i in 1-n range will be the root of tree, numbers that are less than i has to form another binary search tree 
in the left side of the root. And the numbers larger than the root also form the right subtree.
When we multipled the numbers of left subtrees and right subtrees we can find the number of binary search trees with the root i.
...
We should use a vector for store the number of possible trees that consist i element.
*/
public:

    int numTrees(int n) {
    
    //create the vector for memoizing
