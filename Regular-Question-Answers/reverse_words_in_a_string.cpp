...
// Question Link: https://leetcode.com/problems/reverse-words-in-a-string/
    ...
    /* In this problem we wanted to reverse the order of the words with given format
...
public:
    string reverseWords(string &s) {
        ...
        for(int i = 0 ; i < s.size() ; i++ ){
            // If a char is empty then skip this char since extra spaces didn't wanted in the problem
            if(s[i] == ' ') continue;

            // If a nonempty char found then we found starting point of the on of the words
            // From start to end of the word iterate through it and store it into another string "word"  
            string word;
            // Keep adding chars into word until finding empty char which is the end of the word
            while(s[i] != ' ' && i < s.size() ){
                word += s[i] ;
                i++;
            }
        ...
        // Resize function destroy all elements except first n element which is given as parameter
        s.resize(0);
