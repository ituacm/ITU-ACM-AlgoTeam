...
Likewise, there are 2 possible zero-filled subarray containing 2 elements. Finally, there is 1 possible zero-filled subarray that contains 1 element.
This pattern is valid for the rest.
*/
class Solution {
public:
    long long zeroFilledSubarray(vector<int>& nums) {
        ...
        /* As we traverse through the array, we increment zero_count while we encounter a zero. 
        If it is anything else than a zero, meaning that the sequence has ended, we calculate the sum of numbers through 1 to n (where n is the length of zero sequence), 
        then add it to the total subarray count. */
