...
public:
    int lengthOfLongestSubstring(string s) {
        ...
        for (i = 0; i < s.size(); i++) {
            // first check if this character is in the map, if not that's mean never seen so far
            ...
            // assign index to map, so in that way we just keep the last indexes.
            last_index[s[i]] = i;
        }
        // we just updated the longest variable in for loop if there are repeated characters, 
